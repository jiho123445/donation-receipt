...
export const MembershipStatusManager:React.FC<{donations:RawDonationRecord[]}>=({donations})=>{
 ...
 return <main className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
  ...
  <div className="grid grid-cols-1 md:grid-cols-4 gap-4"><Stat icon={<Users/>} label="조회 회원" value={`${classified.length}명`}/><Stat icon={<CheckCircle2/>} label="납부완료" value={`${counts.paid}명`}/><Stat icon={<Clock3/>} label="일부납부" value={`${counts.partial}명`}/><Stat icon={<AlertTriangle/>} label="납부기록 없음" value={`${counts.unpaid}명`}/></div>

...
export const MembershipStatusManager:React.FC<{donations:RawDonationRecord[]}>=({donations})=>{
 ...
 // 연납/일시납 자동 인식: "1년치 회비를 한 번에 낸" 회원은 특정 월에만 기록이 몰려 있어도
 // 실제로는 미납이 아닙니다. 이를 구분하기 위해, 전체 자료(연도 무관)에서 "12개월을 전부
 // 낸 회원"들의 연간 총액 중 가장 흔한 금액을 "연회비 전액"으로 자동 추정하고, 어떤 회원의
 // 선택 연도 총 납부액이 이 금액 이상이면 월이 몇 개만 찍혀 있어도 정상납부로 인정합니다.
 ...
 const classified = rows.map(r => {
   const isLumpSum = annualFullAmount != null && r.total >= annualFullAmount && r.months.length < maxMonth;
   const status = r.months.length >= maxMonth || isLumpSum ? '정상납부' : r.months.length === 0 ? '미납' : '일부납부';
   const missing = status === '정상납부' ? 0 : Math.max(0, maxMonth - r.months.length);
   return { ...r, status, missing, isLumpSum };
 });

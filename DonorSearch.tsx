...
import { Search, UserCheck, Users, Calendar, FileText, CheckCircle2, AlertCircle, ArrowRight, Upload, Sparkles, Building2 } from 'lucide-react';
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1 text-xs text-slate-500">
          ...
          {['홍길동', '김철수', '이영희', '(주)홍천희망기업'].map((name) => (
            <button
              ...
              className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-md border border-slate-200 transition-colors cursor-pointer text-xs"
            >
              {name}
            </button>
          ))}
                ...
                {matchedDonors.map((donor) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-600 max-w-[200px] truncate">{donor.address || '-'}</td>
                    ...
                    {yearDonations.map((d, idx) => (
                      <tr key={d.id || idx} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-2.5 text-slate-600">
                          {d.donationType || orgInfo.donationType || '지정기부금'} ({d.donationCode || orgInfo.donationCode || '40'})
                        </td>
